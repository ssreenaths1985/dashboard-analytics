...
public class RestService {
	...
	@Value("${druid.service.fullpath}")
	private String druidServiceFullPath;
	...
	public JsonNode searchDruid(String aggrQuery) {
		Map<String, Object> requestMap = new HashMap<>();
		requestMap.put(Constants.DruidQueryConstants.QUERY, aggrQuery);
		requestMap.put(Constants.DruidQueryConstants.RESULT_FORMAT, Constants.DruidQueryConstants.OBJECT);
		requestMap.put(Constants.DruidQueryConstants.HEADER, true);
		Map<String, Object> innerMap = new HashMap<>();
		innerMap.put(Constants.DruidQueryConstants.USE_FALLBACK, true);
		innerMap.put(Constants.DruidQueryConstants.SQL_OUTER_LIMIT, 100);
		requestMap.put(Constants.DruidQueryConstants.CONTEXT, innerMap); 
		HttpHeaders headers = getHttpHeaders();
		Gson newGson = new Gson(); 
		HttpEntity<Object> requestEntity = new HttpEntity<>(newGson.toJson(requestMap), headers);
		JsonNode responseNode = null;

		try {
			ResponseEntity<Object> response = retryTemplate.postForEntity(druidServiceFullPath, requestEntity);
			responseNode = new ObjectMapper().convertValue(response.getBody(), JsonNode.class);
		} catch (HttpClientErrorException e) {
